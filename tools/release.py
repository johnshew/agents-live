...
def _check_final_source(accepted_source: str, final_source: str) -> None:
    ...
    collateral = {"AGENTS.md", "CLAUDE.md", ".github/release-channels.toml", manifest,
                  "tools/release.py", "tools/release-report.py", "tools/local-deploy.py",
                  "tools/validate.py", "tools/pre-release-audit.py"}
    if any(path not in collateral and not path.startswith((".agents/", "docs/", "tests/"))
           for path in changed):

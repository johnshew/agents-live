...
class TestReplacingOurOwnExecutable(_TempProject):
    ...
    def test_an_executable_left_behind_by_an_earlier_run_is_swept(self) -> None:
        # A moved-aside executable is still running and cannot be deleted
        # until it exits, so it is left for the next convergence rather
        # than accumulating in the directory a user has on their PATH.
        if sys.platform != "win32":
            self.skipTest("only Windows leaves one behind")
        shim = self._tool_shim()

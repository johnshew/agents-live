...
def resolve_owners(
    agents: Iterable[tuple[str, str]], owners: dict[str, str],
) -> dict[str, str | None]:
    """Map canonical identifiers to owners from the name-keyed registry.

    The current registry schema cannot represent two definitions with the
    same display name. Refuse that repository in registry mode instead of
    silently applying one assignment to both canonical identities.
    """
    by_name: dict[str, str] = {}
    resolved: dict[str, str | None] = {}
    for identifier, name in agents:
        previous = by_name.get(name)
        if previous is not None and previous != identifier:
            raise OwnershipUnavailableError(
                f"ownership registry cannot distinguish duplicate agent "
                f"name {name!r}: {previous}, {identifier}")
        by_name[name] = identifier
        resolved[identifier] = owners.get(name)
    return resolved

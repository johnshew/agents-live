...
`agents-live init` installs completion files for the current user. Windows
installs Bash, Zsh, and PowerShell files in Windows-local user data. Linux,
including WSL, installs Bash and Zsh files in its own XDG data home. Normal
runtime upgrades and `agents-live upgrade --runtime-only` refresh them with
scripts generated by the newly installed CLI. A skills-only upgrade leaves them
unchanged.

The persistent files use the current runtime's XDG data directory. The update
creates missing parent directories and atomically replaces its files:

```text
${XDG_DATA_HOME:-$HOME/.local/share}/bash-completion/completions/agents-live
${XDG_DATA_HOME:-$HOME/.local/share}/zsh/site-functions/_agents-live
${XDG_DATA_HOME:-$HOME/.local/share}/powershell/agents-live-completion.ps1
```

The PowerShell path is created only on Windows. The Windows Bash and Zsh files
support shells installed directly on Windows. A separate Agents Live
installation in WSL owns only the completion files under its Linux home. Neither
runtime inspects or writes the other runtime's home, mounted drives, profiles,
or completion directories.

Run an explicit install or repair when needed:

```bash
agents-live completions --update
```

Generate completion directly from the command spec for the current session:
...
PowerShell does not automatically scan a completion directory. After running
`agents-live completions --update`, add the installed Windows-local script to
your PowerShell profile:

```powershell
$agentsLiveData = if ($env:XDG_DATA_HOME) {
   $env:XDG_DATA_HOME
} else {
   Join-Path $HOME '.local\share'
}
. (Join-Path $agentsLiveData 'powershell\agents-live-completion.ps1')
```

Persistent Bash discovery requires the `bash-completion` framework to be
installed and loaded. Bash itself does not scan the user completion directory.

Zsh discovers completion functions through `fpath`. Add the XDG user
site-functions directory before running `compinit` in `.zshrc`:
...
Agents Live does not edit shell startup files or write to system completion
directories. The profile line is an explicit, one-time user action.

...
| start | in-process | required | schedule, inotify | yes |  | yes | --name, --all, --dry-run, -n, --yes, --transfer-to, --prune-orphans | Activate cron and watcher triggers. |
...
| smoketest | in-process | required | schedule, inotify | yes |  |  | --runtime, --model | Run end-to-end validation. |
...
`agents-live init` installs Bash and Zsh completion files for the current user.
Normal runtime upgrades and `agents-live upgrade --runtime-only` refresh them
with scripts generated by the newly installed CLI. A skills-only upgrade leaves
them unchanged.

The persistent files use the XDG data directory. The update creates missing
parent directories and atomically replaces both files:

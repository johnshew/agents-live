...
Debugging, restarting, and verifying the agents-live infrastructure when
...
## Restarting the agents-live system after a crash

After WSL restarts, cron auto-starts through systemd. Active watchers have
durable `@reboot` crontab entries and should respawn automatically. Use
`agents-live status` to verify convergence; run `start --all` only when repair
is needed.

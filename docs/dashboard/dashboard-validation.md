...
The 10,000-agent profile remains a stress ceiling until open decision 1 in
[dashboard-implementation.md](dashboard-implementation.md) sets it as a
...
- grid and toolbar focus management under collapsed and progressively rendered
  groups;
- visible focus under scrolling and progressive rendering;
- screen-reader names, state, position, sorting, and live action updates;
- withheld actions, confirmation, and partial success;
- contrast, target size, zoom, reflow, light and dark themes, and reduced
  motion;
- focus restoration after details, settings, confirmation, and errors, and
  after a focused row disappears.
...
Requires layout geometry, default grouping, visible logs, settings separation,
and compact-filter acceptance from the installed artifact.
...
- Every P0 requirement has attached passing evidence.
